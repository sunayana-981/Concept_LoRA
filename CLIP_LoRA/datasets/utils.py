...
class DatasetWrapper(TorchDataset):
    ...
    def __getitem__(self, idx):
        ...
        # Support datasets that return (image, label) tuples (e.g. ImageFolder)
        if isinstance(item, (tuple, list)) and len(item) >= 2:
            first = item[0]
            if isinstance(first, str):
                # item[0] is a path
                img0 = read_image(first)
            else:
                # item[0] is already a PIL.Image
                img0 = first
            label = item[1]
            # For tuple-style datasets we don't have impath/domain/classname attributes
            impath = ''
            domain = -1
        else:
            # Fallback to Datum-like objects with attributes
        ...
        if self.transform is None:
            ...
            if isinstance(self.transform, (list, tuple)):
                # apply the first transform as the main returned image; keep compatibility
                imgs = []
                for tfm in self.transform:
                    imgs.append(self._transform_image(tfm, img0))
                img = imgs[0]
            else:
                img = self._transform_image(self.transform, img0)

        if self.return_img0:
            img0_tensor = self.to_tensor(img0)
            # not returned by default, could be extended if needed
