...
def clip_classifier(classnames, template, clip_model):
    """
    Build text features for each class using CLIP text encoder.
    - template can be a string or a list of template strings (e.g. "a photo of a {}.")
    - returns a tensor of shape (D, num_classes) on the same device as the clip_model
    """
    ...
    tokenized = clip.tokenize(texts)  # uses project clip.tokenize
    device = next(clip_model.parameters()).device
    tokenized = tokenized.to(device)

    with torch.no_grad():
        text_feats = clip_model.encode_text(tokenized)  # (T, D)
    ...
    for _ in range(C):
        reps = []
        for _ in templates:
            reps.append(text_feats[idx])
            idx += 1
        rep = torch.stack(reps, dim=0).mean(dim=0)
        rep = rep / rep.norm()
        per_class.append(rep)

    # stack to shape (D, C)
    textual_features = torch.stack(per_class, dim=0).t().to(device)
