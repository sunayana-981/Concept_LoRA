...
def train_linear_probe(train_latents, train_labels, test_latents, test_labels,
    ...
    which is memory-efficient for high-dimensional sparse SAE latents
    (49k dims × 90k samples would OOM with LBFGS).

    Returns:
        test_acc: float (percentage)
        train_acc: float (percentage)
    """
    # MaxAbsScaler scales each feature by its max abs value — no centering,
    # so it stays sparse-friendly and doesn't double memory like StandardScaler.
    scaler = MaxAbsScaler()
