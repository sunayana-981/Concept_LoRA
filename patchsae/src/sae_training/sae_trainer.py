...
class SAETrainer:
    ...
    def _calculate_metrics(
        ...
        total_variance = sae_in.pow(2).sum(-1).squeeze()
        ev_per_sample = 1 - per_token_l2_loss / total_variance.clamp(min=1e-8)

        ev_mean = ev_per_sample.mean().item()
        # std requires ≥2 elements; guard against scalar/single-sample batches
        ev_std = ev_per_sample.std().item() if ev_per_sample.numel() > 1 else 0.0

        return {
            "metrics/explained_variance": ev_mean,
            "metrics/explained_variance_std": ev_std,
            "metrics/l0": l0.item(),
        }
