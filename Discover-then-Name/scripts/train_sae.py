...
args.data_dir_activations = {
    'img': '/home/sunayana/Documents/Concept_LoRA/datasets/pets_feats_vitb16'
}
print(f"[HACK] Overriding data_dir_activations to: {args.data_dir_activations['img']}")
# --- END HACK ---

# --- NEW HACK (START) ---
# Set this path to your pre-trained model.
# If you set it to None, the script will train from scratch as before.
PRETRAINED_SAE_PATH = "/home/sunayana/Documents/Concept_LoRA/Discover-then-Name/pretrained/Checkpoints/clip_ViTB16_sparse_autoencoder_final.pt"
# --- NEW HACK (END) ---
...
# --- NEW HACK (START) ---
# Load the pre-trained model's weights if the path is set
if PRETRAINED_SAE_PATH:
    if not os.path.isfile(PRETRAINED_SAE_PATH):
        print(f"[warn] PRETRAINED_SAE_PATH specified but file not found: {PRETRAINED_SAE_PATH}")
        print("[warn] Proceeding to train from scratch.")
    else:
        print(f"[HACK] Loading pre-trained model from: {PRETRAINED_SAE_PATH}")
        try:
            # Load the checkpoint
            checkpoint = torch.load(PRETRAINED_SAE_PATH, map_location=args.device)
            
            # Check if it's the standardized format (dict with 'state_dict')
            if isinstance(checkpoint, dict) and "state_dict" in checkpoint:
                model_state_dict = checkpoint["state_dict"]
                meta = checkpoint.get("meta", {})
                
                # Optional: Check dimensions for safety
                if meta:
                    meta_D = meta.get("n_input_features")
                    meta_F = meta.get("n_learned_features")
                    meta_C = meta.get("n_components") # Can be None
                    script_C = n_components # Is always int >= 1

                    if meta_D is not None and meta_D != autoencoder_input_dim:
                        raise ValueError(f"Input dim mismatch: Model has {meta_D}, script expects {autoencoder_input_dim}")
                    if meta_F is not None and meta_F != n_learned_features:
                        raise ValueError(f"Learned features mismatch: Model has {meta_F}, script expects {n_learned_features}")
                    
                    if meta_C is None and script_C == 1:
                         print("[warn] Model has n_components=None, script has n_components=1. Assuming compatibility.")
                    elif meta_C is not None and meta_C != script_C:
                         raise ValueError(f"Component mismatch: Model has {meta_C}, script expects {script_C}")

                print("[HACK] Pre-trained model dimensions match. Loading state_dict.")
                autoencoder.load_state_dict(model_state_dict)
            
            # Fallback for raw state_dict
            elif isinstance(checkpoint, dict):
                print("[warn] Loaded checkpoint does not have 'state_dict' key. Assuming raw state_dict.")
                autoencoder.load_state_dict(checkpoint)
            
            else:
                raise ValueError("Loaded checkpoint is not a dictionary (state_dict).")

            print(f"[HACK] Successfully loaded pre-trained model at {time()-start_time:.2f}s")
        
        except Exception as e:
            print(f"[error] FAILED to load pre-trained model: {e}")
            print("[error] ABORTING. Please check the model path or set PRETRAINED_SAE_PATH to None.")
            exit(1) # Exit if loading fails
else:
    print("[HACK] PRETRAINED_SAE_PATH is not set. Training from scratch.")
# --- NEW HACK (END) ---
...
print(f"DEBUG: Pipeline created at {time() :.2f}s")
