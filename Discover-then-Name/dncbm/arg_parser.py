...
def get_common_parser():
    ...
    # New CLI overrides to allow datasets other than CC3M
    parser.add_argument(
        "--embeddings_path",
        type=str,
        default=None,
        help="Optional path to the vocab/embeddings .pth used for naming (overrides default).",
    )
    parser.add_argument(
        "--data_dir_activations",
        type=str,
        default=None,
        help="Optional path (string) to directory containing activation .pt files for the chosen modality. If provided as a string it will override mapping created by common_init.",
    )
    parser.add_argument(
        "--ae_input_dim",
        type=int,
        default=None,
        help="Optional override for autoencoder input dimension (bypass autoencoder_input_dim_dict).",
    )
